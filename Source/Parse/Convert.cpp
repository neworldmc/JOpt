...
lookup_string(const std::vector<StringTableEntry> &table, int key)
{
    int lo = 0;
    int hi = table.size();
    if (hi == 0 || key < table[lo].key || key > table[hi-1].key) {
        throw NotFound(std::to_string(key));
    }
    for (;;) {
        if (lo == hi) {
            throw NotFound(std::to_string(key));
...
ConvertClassFile(const Parse::ClassFile *cf, Region &r)
{
    ...
    for (int i=1; i<n_cpinfo; i++) {
        const CpInfoBase *cpinfo = cf->ConstantPool[i].get();
        if (cpinfo->Tag == CPoolTags::Utf8) {
            const auto utf8info = (const ConstantUtf8Info *) cpinfo;
    ...
    auto get_class = [cf,&strtab](uint16_t index) {
        if (index <= 0 || index >= cf->ConstantPoolCount) {
            throw InvalidCPIndex(std::to_string(index));
        }
        const CpInfoBase *cpinfo = cf->ConstantPool[index].get();
        if (cpinfo->Tag != CPoolTags::Class) {
            throw InvalidCPIndex("#" + std::to_string(index) + " is not Class");
        }
        auto classinfo = (const ConstantClassInfo *) cpinfo;
    ...
    auto convert_field_info = [&strtab,&r,&convert_attribute_info](const FieldInfo &fi, Field &f) {
        ...
        for (int i=0; i<n; i++) {
            convert_attribute_info(fi.Attributes[i], f.Attributes[i]);
        }
    };
    ...
    auto convert_method_info = [&strtab,&r,&convert_attribute_info](const MethodInfo &mi, Method &m) {
        ...
        for (int i=0; i<n; i++) {
            convert_attribute_info(mi.Attributes[i], m.Attributes[i]);
        }
    };
